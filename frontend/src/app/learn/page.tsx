...
const phases: Phase[] = [
    {
        id: 1,
        title: "Foundations",
        subtitle: "The building blocks of bond markets",
        terms: [
            {
                term: "Bond",
                category: "Basics",
    ...
    },
    {
        ...
        subtitle: "How risk works and how to measure it",
        terms: [
            {
                term: "Duration",
                category: "Risk",
    ...
    },
    {
        ...
        subtitle: "The math that powers smart portfolios",
        terms: [
            {
                term: "Yield Curve",
                category: "Basics",
                ...
                example: "Right now: 1-year Treasury pays about 4.2%, 5-year pays 4.0%, 10-year pays 4.3%, 30-year pays 4.6%. Plot those and you have the yield curve — the line at the top of the dashboard.",
...
export default function LearnPage() {
    ...
    const activeTerm: Term | undefined = expandedTerm
        ? phases.flatMap((p) => p.terms).find((t) => t.term === expandedTerm)
        : undefined;

    return (
        <div className="min-h-screen spine mx-auto max-w-[1280px]" style={{ background: "var(--paper)" }}>
            {/* ── Masthead ── */}
            <section className="px-6 pt-24 pb-12 w-full">
                <p className="mono-label mb-6">
                    {totalTerms} concepts · 4 phases · beginner friendly
                </p>
                <h1
                    className="text-4xl md:text-5xl font-semibold mb-4 max-w-[16ch]"
                    style={{ fontFamily: "'Fraunces', Georgia, serif", color: "var(--ink)", letterSpacing: "-0.02em" }}
                >
                    A reading path through the bond market.
                </h1>
                <p
                    className="text-lg leading-relaxed max-w-[62ch]"
                    style={{ color: "var(--muted)" }}
                >
                    From bond basics to advanced risk analytics. Every concept is
                    explained as if it&apos;s your first day in finance — read top to
                    bottom, then go run it yourself.
                </p>
            </section>

            {/* ── Phase index ── */}
            <nav
                className="sticky top-16 z-50 px-6 py-3 w-full"
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                    {phases.map((phase) => (
                        <button
                            ...
                            className="mono-label transition-colors duration-150"
                            style={{ color: "var(--muted)" }}
                            onMouseEnter={(e) => (e.currentTarget.style.color = "var(--ink)")}
                            onMouseLeave={(e) => (e.currentTarget.style.color = "var(--muted)")}
                        >
                            {pad(phase.id)} · {phase.title}
                        </button>
                    ))}
                </div>
            </nav>
            ...
            {/* ── Phases (broadsheet) ── */}
            <section className="px-6 pt-12 pb-24 w-full">
                {phases.map((phase) => (
                    <div key={phase.id} id={`phase-${phase.id}`} className="scroll-mt-32 mb-20">
                        {/* Phase header */}
                        <motion.div
                            className="flex items-baseline gap-4 pb-4 mb-8"
                        ...
                        {/* Terms — annotated entries with marginal category */}
                        <div className="divide-y" style={{ borderColor: "var(--hairline)" }}>
                            {phase.terms.map((t, ti) => (
                                <motion.button
                                    key={t.term}
                                    onClick={() => setExpandedTerm(t.term)}
                                    className="press w-full text-left grid md:grid-cols-[120px_1fr] gap-x-8 gap-y-2 py-6 group"
                                    style={{ borderColor: "var(--hairline)" }}
                                    initial={{ opacity: 0, y: 8 }}
                                    whileInView={{ opacity: 1, y: 0 }}
                                    viewport={{ once: true, margin: "-40px" }}
                                    transition={{ duration: 0.3, ease: EASE_OUT, delay: ti * 0.05 }}
                                >
                                    <div className="mono-label pt-1">{t.category}</div>
                                    <div className="max-w-[62ch]">
                                        <div className="flex items-baseline gap-3">
                                            ...
                                            <span className="mono-label opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                                                Read →
                                            </span>
                                        </div>
                                        <p
                ...
                {/* End marker */}
                <div className="pt-8" style={{ borderTop: "1px solid var(--hairline-strong)" }}>
                    <h2
                    ...
                    <p className="text-base mb-6 max-w-[62ch]" style={{ color: "var(--muted)" }}>
                        You&apos;ve read all {totalTerms} concepts. Now open the dashboard
                        and optimize a real portfolio with what you know.
                    </p>
                    <Link
                        href="/dashboard"
                        className="press inline-block px-7 py-3.5 text-sm font-medium transition-colors duration-150"
            ...
            {activeTerm && (
                ...
                >
                    ...
                    >
                        ...
                        >
                            <div>
                                <div className="mono-label mb-2">{activeTerm.category}</div>
                                <h2
                                    className="text-2xl font-semibold"
                                    style={{ fontFamily: "'Fraunces', Georgia, serif", color: "var(--ink)" }}
                                >
                            ...
                            <button
                                onClick={() => setExpandedTerm(null)}
                                className="press mono-label flex-shrink-0 pt-1"
                                aria-label="Close"
                            >
                                Close ✕
                            </button>
                        ...
                        <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
                            <div>
                                <div className="mono-label mb-2">Explained simply</div>
                                <p className="text-base leading-relaxed" style={{ color: "var(--ink)" }}>
                                    {activeTerm.explanation}
                                </p>
                            </div>
                            <div className="p-4" style={{ borderLeft: "1px solid var(--hairline-strong)" }}>
